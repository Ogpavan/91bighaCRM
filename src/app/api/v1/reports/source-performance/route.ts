...
import { getSourcePerformanceReport, parseReportFilters } from "@/lib/report-aggregates";
...
export async function GET(request: Request) {
  try {
    ...
    const data = await getSourcePerformanceReport(parseReportFilters(request));
    return withCors(NextResponse.json({ success: true, data }), request);
  } catch (error) {
    const status = error instanceof AuthError ? error.status : 400;
    const message = error instanceof Error ? error.message : "Unable to load source performance.";
